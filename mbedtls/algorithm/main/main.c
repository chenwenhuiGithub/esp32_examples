...
void rsa_decrypt(uint8_t *ciphertext, uint32_t ciphertext_len, mbedtls_mpi *N, mbedtls_mpi *P, mbedtls_mpi *Q, mbedtls_mpi *D, mbedtls_mpi *E, uint8_t *plaintext, uint32_t *plaintext_len) {
    const uint32_t KEY_SIZE = 2048;
    uint32_t i = 0;
...
#endif
    while (i < blocks) {
        mbedtls_rsa_pkcs1_decrypt(&rsa_ctx, mbedtls_ctr_drbg_random, &ctr_drbg, &dec_len, ciphertext + (i * (KEY_SIZE / 8)), plaintext + total_dec_len, KEY_SIZE / 8);
        total_dec_len += dec_len; // dec_len = BLOCK_SIZE/.../BLOCK_SIZE/remainder
        i++;
    }
    // if (remainder) {
...
void rsa_pk_encrypt(uint8_t *plaintext, uint32_t plaintext_len, uint8_t *pubkey, uint32_t pubkey_len, uint8_t *ciphertext, uint32_t *ciphertext_len) {
    const uint32_t KEY_SIZE = 2048;
#if CONFIG_RSA_PADDING_PKCS1_V21 == 1
    const uint32_t BLOCK_SIZE = (KEY_SIZE / 8) - 66; // PKCS#1_v2.1 padding size = 66(2 * sizeof(hash) + 2)
#else
...
#endif
    while (i < blocks) {
        mbedtls_pk_encrypt(&pk_ctx, plaintext + (i * BLOCK_SIZE), BLOCK_SIZE, ciphertext + total_enc_len, &enc_len, KEY_SIZE / 8, mbedtls_ctr_drbg_random, &ctr_drbg);
        total_enc_len += enc_len; // enc_len = KEY_SIZE / 8
        i++;
    }
    if (remainder) {
        mbedtls_pk_encrypt(&pk_ctx, plaintext + (i * BLOCK_SIZE), remainder, ciphertext + total_enc_len, &enc_len, KEY_SIZE / 8, mbedtls_ctr_drbg_random, &ctr_drbg);
        total_enc_len += enc_len; // enc_len = KEY_SIZE / 8
    }
    mbedtls_pk_free(&pk_ctx);
...
void rsa_pk_decrypt(uint8_t *ciphertext, uint32_t ciphertext_len, uint8_t *privkey, uint32_t privkey_len, uint8_t *plaintext, uint32_t *plaintext_len) {
    const uint32_t KEY_SIZE = 2048;
    uint32_t i = 0;
    uint32_t blocks = ciphertext_len / (KEY_SIZE / 8); // multiple of KEY_SIZE / 8
    // uint32_t remainder = ciphertext_len % (KEY_SIZE / 8); // = 0
...
#endif
    while (i < blocks) {
        mbedtls_pk_decrypt(&pk_ctx, ciphertext + (i * (KEY_SIZE / 8)), KEY_SIZE / 8, plaintext + total_dec_len, &dec_len, KEY_SIZE / 8, mbedtls_ctr_drbg_random, &ctr_drbg);
        total_dec_len += dec_len; // dec_len = BLOCK_SIZE/.../BLOCK_SIZE/remainder
        i++;
    }
    // if (remainder) {
...
void rsa_pk_sign(uint8_t *hash, uint32_t hash_len, uint8_t *privkey, uint32_t privkey_len, uint8_t *sign, uint32_t *sign_len) {
    mbedtls_entropy_context entropy;
    mbedtls_ctr_drbg_context ctr_drbg;    
    mbedtls_pk_context pk_ctx;
    ...
    mbedtls_rsa_set_padding(mbedtls_pk_rsa(pk_ctx), MBEDTLS_RSA_PKCS_V21, MBEDTLS_MD_SHA256);
#endif
    mbedtls_pk_sign(&pk_ctx, MBEDTLS_MD_SHA256, hash, hash_len, sign, 256, (size_t *)sign_len, mbedtls_ctr_drbg_random, &ctr_drbg); // KEY_SIZE / 8
    mbedtls_pk_free(&pk_ctx);
...
void ecc_pk_sign(uint8_t *hash, uint32_t hash_len, uint8_t *privkey, uint32_t privkey_len, uint8_t *sign, uint32_t *sign_len) {
    mbedtls_entropy_context entropy;
    mbedtls_ctr_drbg_context ctr_drbg;
    mbedtls_pk_context pk_ctx;
    ...
    mbedtls_pk_sign(&pk_ctx, MBEDTLS_MD_SHA256, hash, hash_len, sign, 256, (size_t *)sign_len, mbedtls_ctr_drbg_random, &ctr_drbg); // der_header + 2 * grp.bit_size / 8
    mbedtls_pk_free(&pk_ctx);
...
void x509_crt_sign(uint8_t *subject_pubkey, uint32_t subject_pubkey_len, char *subject_name, char *issuer_name, char *sn, char *not_before, char *not_after,
                   uint8_t *issuer_privkey, uint32_t issuer_privkey_len, uint8_t is_ca, uint8_t *crt, uint32_t size, uint32_t *crt_len) {
    mbedtls_entropy_context entropy;
    mbedtls_ctr_drbg_context ctr_drbg;
...
#else
    *crt_len = mbedtls_x509write_crt_der(&x509write, crt, size, mbedtls_entropy_func, &entropy); // write at the end of buffer
...
void test_rsa_crypt() {
    ...
    rsa_decrypt(encode, encode_len, &N, &P, &Q, &D, &E, decode, &decode_len);
...
void test_rsa_pk_crypt()
{
    ...
    rsa_pk_encrypt(src_data, sizeof(src_data), pubkey, pubkey_len, encode, &encode_len);
    ESP_LOGI(TAG, "rsa encode data, len:%lu", encode_len); // multiple(blocks or blocks + 1) of KEY_SIZE / 8
    ESP_LOG_BUFFER_HEX(TAG, encode, encode_len);

    rsa_pk_decrypt(encode, encode_len, privkey, privkey_len, decode, &decode_len);
    ESP_LOGI(TAG, "rsa decode data, len:%lu", decode_len); // src_len
    ESP_LOG_BUFFER_HEX(TAG, decode, decode_len);
...
void test_rsa_pk_sign() {
    ...
    rsa_pk_sign(hash, hash_len, privkey, privkey_len, sign, &sign_len);
    ESP_LOGI(TAG, "rsa sign data, len:%lu", sign_len); // KEY_SIZE / 8
    ESP_LOG_BUFFER_HEX(TAG, sign, sign_len);
...
void test_ecc_pk_sign() {
    ...
    ecc_pk_sign(hash, hash_len, privkey, privkey_len, sign, &sign_len);
