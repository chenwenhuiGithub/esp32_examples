...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (WIFI_EVENT == event_base) {
        ...
        case WIFI_EVENT_AP_START:
            ESP_LOGI(TAG, "WIFI_EVENT_AP_START, ssid:%s channel:%u", CONFIG_WIFI_AP_SSID, CONFIG_WIFI_AP_CHANNEL);
            break;
        ...
        case WIFI_EVENT_AP_STACONNECTED:
            ...
            ESP_LOGI(TAG, "WIFI_EVENT_AP_STACONNECTED, aid:0x%04x mac:%02x:%02x:%02x:%02x:%02x:%02x",
                evt_ap_staconn->aid, evt_ap_staconn->mac[0], evt_ap_staconn->mac[1],
                evt_ap_staconn->mac[2], evt_ap_staconn->mac[3], evt_ap_staconn->mac[4], evt_ap_staconn->mac[5]); 
            break;
        case WIFI_EVENT_AP_STADISCONNECTED:
            evt_ap_stadisconn = (wifi_event_ap_stadisconnected_t *)event_data; // reason:wifi_err_reason_t
            ESP_LOGI(TAG, "WIFI_EVENT_AP_STADISCONNECTED, reason:0x%02x aid:0x%04x mac:%02x:%02x:%02x:%02x:%02x:%02x",
                evt_ap_stadisconn->reason, evt_ap_stadisconn->aid, evt_ap_stadisconn->mac[0], evt_ap_stadisconn->mac[1],
                evt_ap_stadisconn->mac[2], evt_ap_stadisconn->mac[3], evt_ap_stadisconn->mac[4], evt_ap_stadisconn->mac[5]);
            break;
    ...
    if (IP_EVENT == event_base) {
        ...
        case IP_EVENT_AP_STAIPASSIGNED:
            ...
            ESP_LOGI(TAG, "IP_EVENT_AP_STAIPASSIGNED, ip:" IPSTR " mac:%02x:%02x:%02x:%02x:%02x:%02x",
                IP2STR(&evt_ap_ip_assigned->ip), evt_ap_ip_assigned->mac[0], evt_ap_ip_assigned->mac[1],
                evt_ap_ip_assigned->mac[2], evt_ap_ip_assigned->mac[3], evt_ap_ip_assigned->mac[4], evt_ap_ip_assigned->mac[5]);
            break;
        default:
