...
#define CONFIG_WIFI_SCAN_AP_SIZE                5
...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    uint16_t ap_size = CONFIG_WIFI_SCAN_AP_SIZE;
    uint16_t ap_num = 0;
    uint16_t i = 0;
    ...
    if (WIFI_EVENT == event_base) {
        ...
        case WIFI_EVENT_SCAN_DONE:
            ...
            for (i = 0; (i < ap_num) && (i < CONFIG_WIFI_SCAN_AP_SIZE); i++) {
                ESP_LOGI(TAG, "bssid:%02x:%02x:%02x:%02x:%02x:%02x\t channel:%u\t rssi:%d\t authmode:0x%02x\t ssid:%s",
                    ...
                    ap_records[i].primary, ap_records[i].rssi, ap_records[i].authmode, ap_records[i].ssid);
            }
            break;
        ...
        case WIFI_EVENT_STA_CONNECTED:
            ...
            ESP_LOGI(TAG, "WIFI_EVENT_STA_CONNECTED, channel:%u authmode:0x%02x aid:0x%04x bssid:%02x:%02x:%02x:%02x:%02x:%02x",
                evt_sta_conn->channel, evt_sta_conn->authmode, evt_sta_conn->aid, evt_sta_conn->bssid[0], evt_sta_conn->bssid[1],
                evt_sta_conn->bssid[2], evt_sta_conn->bssid[3], evt_sta_conn->bssid[4], evt_sta_conn->bssid[5]);
            break;
