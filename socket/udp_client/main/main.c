...
#define CONFIG_UDP_SERVER_IP                    "192.168.108.112"
#define CONFIG_UDP_SERVER_PORT                  60001
#define CONFIG_UDP_LOCAL_PORT                   60002
#define CONFIG_UDP_RECV_TIMEOUT_MS              3000


static const char *TAG = "udp_client";

static void udp_client_cb(void *pvParameters) {
    int sock = 0;
    int err = 0;
    int rx_len = 0;
    ...
    struct timeval tv = {
        .tv_sec = CONFIG_UDP_RECV_TIMEOUT_MS / 1000,
        .tv_usec = (CONFIG_UDP_RECV_TIMEOUT_MS % 1000) * 1000
    };
    ...
    local_addr.sin_port = htons(CONFIG_UDP_LOCAL_PORT);
    ...
    server_addr.sin_addr.s_addr = inet_addr(CONFIG_UDP_SERVER_IP);
    server_addr.sin_port = htons(CONFIG_UDP_SERVER_PORT);
    err = sendto(sock, "hello world", strlen("hello world"), 0, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (err < 0) {
...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (IP_EVENT == event_base) {
        ...
        case IP_EVENT_STA_GOT_IP:
            ...
            xTaskCreate(udp_client_cb, "udp_client", 4096, NULL, 5, NULL);
            break;
