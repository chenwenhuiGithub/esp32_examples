...
#define CONFIG_ADC_PERIOD_MS                    5000
...
void bat_adc_cb(void* parameter) {
    int raw_data = 0, voltage_mv = 0;
    char voltage_format[16] = {0};

    while (1) {
        raw_data = 0;
        adc_oneshot_read(s_hd_unit, CONFIG_ADC_CHANNEL, &raw_data);
        adc_cali_raw_to_voltage(s_hd_cali, raw_data, &voltage_mv);
        snprintf(voltage_format, sizeof(voltage_format) - 1, "%4d mV 0x%03X", voltage_mv, raw_data);
        ESP_LOGI(TAG, "%s", voltage_format);
        // oled_show_string(0, 0, voltage_format, CHAR_SIZE_8X16);
        vTaskDelay(pdMS_TO_TICKS(CONFIG_ADC_PERIOD_MS));
    }
}
...
void app_main() {
    ...
    xTaskCreate(bat_adc_cb, "bat_adc", 4 * 1024, NULL, 3, NULL);
