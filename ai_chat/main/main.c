...
void battery_adc_cb(void* parameter) {
    esp_err_t err = ESP_OK;
    ...
    while (1) {
        ...
        snprintf(voltage_format, sizeof(voltage_format) - 1, "vol:%d", voltage_mv);
...
void app_main() {
    ...
    xTaskCreate(battery_adc_cb, "battery_adc", 2048, NULL, 3, NULL);
